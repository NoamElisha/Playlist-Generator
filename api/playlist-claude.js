...
// Playlist generator: strict rules for language, locale, and genre.
...
5. If the user provides fewer than REQUESTED_DISTINCT_ARTISTS seed artists,
   complete the missing artists with others from the same genre and same locale as the majority of seeds.
...
8. Language and locale rules:
   - If the seed songs are mostly Hebrew, ALL generated songs must be Hebrew songs by Israeli artists.
   - If the seed songs are mostly English, ALL generated songs must be by international English-language artists. Do NOT include Israeli artists or Hebrew titles in this case.
9. Genre rules:
   - Stay strictly in the same genre/vibe as the seeds.
   - Do not mix Mizrahi/Israeli pop with international pop, or vice versa.
...
If seeds are predominantly English: generate ONLY international English-language songs (no Hebrew, no Israeli artists).
Stay strictly in the same genre/vibe as the seeds.`;
